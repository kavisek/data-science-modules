...
-- Schema db_vphan
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `db_vphan` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `db_vphan` ;

-- -----------------------------------------------------
-- Table `db_vphan`.`Station`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Station` (
...
-- -----------------------------------------------------
-- Table `db_vphan`.`Store`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Store` (
...
-- -----------------------------------------------------
-- Table `db_vphan`.`Department`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Department` (
...
-- Table `db_vphan`.`Item`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Item` (
...
-- -----------------------------------------------------
-- Table `db_vphan`.`Inventory`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Inventory` (
...
-- Table `db_vphan`.`Sale`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Sale` (
  `sale_id` INT NOT NULL AUTO_INCREMENT COMMENT '',
  `sale_date` DATETIME NULL COMMENT '',
  `sale_total` VARCHAR(45) NULL COMMENT '',
  `store_id` INT NOT NULL COMMENT '',
...
-- -----------------------------------------------------
-- Table `db_vphan`.`Weather Event`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Weather Event` (
...
-- -----------------------------------------------------
-- Table `db_vphan`.`Line_Item`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_vphan`.`Line_Item` (
...
LOAD DATA LOCAL INFILE "C:/Users/Vivian/Desktop/BUS 464/Project/key.csv"
into TABLE `key`
...
LOAD DATA LOCAL INFILE "C:/Users/Vivian/Desktop/BUS 464/Project/train.csv"
into TABLE `train`
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
ignore 1 lines;
...
LOAD DATA LOCAL INFILE "C:/Users/Vivian/Desktop/BUS 464/Project/weather.csv"
...
LOAD DATA LOCAL INFILE "C:/Users/Vivian/Desktop/store.csv"
...
LOAD DATA LOCAL INFILE "C:/Users/Vivian/Desktop/item.csv"
into TABLE `itemtemp`
FIELDS TERMINATED BY ','
LINES TERMINATED BY '\n'
ignore 1 lines;
...
LOAD DATA LOCAL INFILE "C:/Users/Vivian/Desktop/department.csv"
...
#calculate and insert sale totals into saletemp table
insert into saletemp (sale_total)
select sum(item_unit_price * line_units)
from item i, line_item l, sale s
where i.item_id = l.item_id
and l.sale_id = s.sale_id
group by sale_id
order by sale_id;

#load data into sale table
insert into sale (sale_total)
select sale_total
from saletemp;
